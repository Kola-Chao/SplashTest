...
package com.buzz.shortnews.flashnews.core.designsystem.tab
...
fun Tab(
    ...
    val styledText: @Composable (() -> Unit)? =
        text?.let {
            @Composable {
                ProvideTextStyle(TextStyle(), content = text)
            }
        }
    Tab(
        selected,
...
fun LeadingIconTab(
    ...
    val ripple = ripple(true, Dp.Unspecified, selectedContentColor)
...
private fun Placeable.PlacementScope.placeTextAndIcon(
    ...
    val textOffset =
        with(density) {
            baselineOffset.roundToPx() +3.dp.roundToPx()
        }
